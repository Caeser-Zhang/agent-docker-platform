...
printf "backend  http://localhost:3000/api/health -> "
curl -s --max-time 10 http://localhost:3000/api/health; echo
printf "frontend http://localhost:3000            -> "
curl -s -o /dev/null -w '%{http_code}\n' --max-time 10 http://localhost:3000

echo "== UI bundle sanity (spot-check a few feature strings) =="
js=$(curl -s --max-time 10 http://localhost:3000/ | grep -oE 'assets/index-[^"]+\.js' | head -1)
if [ -n "$js" ]; then
  echo "bundle: $js"
  curl -s --max-time 10 "http://localhost:3000/$js" > /tmp/bundle.js
  for s in "总是允许" "重命名会话" "Agent 需要你的输入" "新增个人 MCP Server" "files/download"; do
