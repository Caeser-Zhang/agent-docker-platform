...
#
# 代码类后缀与容器内置插件 present-file/index.js 的 CODE_MIME_BY_EXT 保持一致
# （跨语言无法共享常量，故重复一份）。两边都必须列全：present_file 交付 .py/.c
# 这类文件时，这里缺项只会让 mime 退化成 application/octet-stream，前端仍按纯
# 文本渲染，但信令与实际返回的类型就不一致了。
MIME_BY_EXT = {
    ".html": "text/html",
    ".htm": "text/html",
    ".md": "text/markdown",
