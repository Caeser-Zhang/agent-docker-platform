---
name: plantuml
description: Generate PlantUML diagram sources, especially types Mermaid cannot do easily (Salt wireframes, use case, timing, ArchiMate, nwdiag, WBS). Use when asked for a wireframe or PlantUML source, or to extract puml from markdown. On this platform the skill produces .puml source files only — no local renderer is installed. Class, ER, state, sequence, and C4 default to design-doc-mermaid on GitHub wiki.
---

# PlantUML Diagram Generation and Conversion

> **Platform note (agent container):** No Java, Graphviz, or plantuml.jar is
> installed here, and none will be — by design, rendering does not happen in
> the user container. Use this skill to **author `.puml` source files** into
> the workspace. Ignore the conversion/rendering instructions below
> (`check_setup.py`, `convert_puml.py`, `process_markdown_puml.py`,
> `java -jar …`); they will fail with "PlantUML not found". Deliver the
> `.puml` files and tell the user to render them in an IDE PlantUML plugin,
> the PlantUML web server, or CI.
