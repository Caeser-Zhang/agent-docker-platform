...
**On this platform, run every script with the `mermaid-node` command** (a pre-installed isolated Node runtime with the skill's dependencies pre-baked) instead of bare `node`, which is not on PATH here. Dependencies are already in place — never run `npm install`.
...
```bash
mermaid-node scripts/themes.mjs
```

### Render SVG

```bash
mermaid-node scripts/render.mjs \
  --input diagram.mmd \
  --output diagram.svg \
  --theme tokyo-night
```
...
```bash
mermaid-node scripts/batch.mjs \
  --input-dir ./diagrams \
  --output-dir ./rendered \
  --format svg \
  --theme github-dark \
...
Run `mermaid-node scripts/render.mjs --help` or `mermaid-node scripts/batch.mjs --help` for the authoritative CLI list.
...
Run both `mermaid-node scripts/smoke-test.mjs` and `mermaid-node scripts/validate-docs.mjs` when changing this skill, its scripts, templates, or references.

## Troubleshooting

- Missing dependency: dependencies are pre-baked on this platform and linked into `<skill-root>/node_modules` at container boot — do NOT run `npm install` (npm does not exist here). If an import still fails, verify that `node_modules` symlink exists in `<skill-root>` and report it as a platform issue.
- Unknown theme: run `mermaid-node scripts/themes.mjs` and use an exact listed name.
