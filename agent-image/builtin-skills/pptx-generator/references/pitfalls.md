...
```bash
pptx-markitdown output.pptx | grep -iE "xxxx|lorem|ipsum|placeholder|this.*(page|slide).*layout"
```
...
1. Generate slides -> Extract text with `pptx-markitdown output.pptx` -> Review content
...
```bash
pptx-markitdown slide-XX-preview.pptx
```
